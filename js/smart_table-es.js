...
function filter(table)
{
    ...
    lable +="<table class='sstr' align='center'><tr><th>Lables</th><th>Filtrar texto</th></tr>";
...
function sort_ch(table)
{
    ...
    {
        jAlert('Se necesita seleccionar una columna como mínimo.','Mensaje');			
        return false;
    }
    ...
    lable +="<table class='sstr' align='center'><tr><th>Lables</th><th>Ascendente</th><th>Descendiente</th></tr>";
    jQuery(".fil_left > div[id]").each(function(){
        var context = $(this);
        var inp=context.attr('id');
        var sty=inp.split('_');
...
{	
    jPrompt1('Email Id:', '', 'Enviar Correo', function(r) {
        if( r ) 
        {
            CreateTable(table);
            ...
            $.ajax({
				...
				$('#loading').hide();
            $("body").css("opacity","1");; 
                    jAlert('Correo Enviado','Mensaje');       
        }
           
    });
}
	
function mailtoview(table)
{   
    Createpdfview(table);
    jPrompt1('Email Id:', '', 'Enviar Correo', function(r) {
        if( r ) 
        {
            ...
            var data=tableToCSV('#export_table_view_pdf');
            var tap=table.split('_');                    
            var datas="mail_to="+r+"&name="+$('#'+tap[0]).attr('alt')+"&table_data="+$('#export_table_view_pdf').html();
            $.ajax({
                type: "POST",
                url: "common/tablemail",
                data: datas,
...
function table_cells(table)
{
    ...
    $(sort_main_div).find('#title').text('Columnas de Tabla');
        ...
        $(sort_main_div).find('.to_left').unbind('click').click(function()
        {
			...
			//alert($('.fil_left').children('div').length);
                if($(sort_main_div).find('.fil_left').children('div').length < hlen)
					...
					jAlert('La selección máxima de columnas no debe ser más de'+hlen,'Mensaje');
    ...
    $('.cells').click(function()
    {
        if($(window).width()>1026)
        {
            ...
            if($('.table_sel').length>5)
            {
                $.cookie('table_show','4')
                $(this).removeClass('table_sel');
                jAlert('La selección máxima de columnas no debe ser más de cinco','Menhsaje',function(r)
                {
                    });
            }
        }
        ...
        if($(window).width()<1030)
        {
            ...
            {
                $(this).removeClass('table_sel');
                jAlert('La selección máxima de columnas no debe ser más de tres.','Mensaje');
            }
        }
        if($(window).width()<600)
        {
...
function sorte(table)
{
    ...
    $('#title').text('Clasificación Múltiple');
    $('#sort_pop').show();
    $('.contnt').show();
    $('.cont').hide();
    $('#stt').show();
...
function ssum(Column,table,ids)
{
	...
	{
		jAlert('<b>Campo seleccionado no válido</b>', 'Mensaje');
	}
	else
	{
   $('#'+table+' td:nth-child('+ids+')').each(function(index, element) {
    ...
    if(tog==0)
    {
        jAlert('<b>Total: </b>'+total, 'Mensaje');
    }
    else
    {
        jAlert('<b>Campo seleccionado no válido</b>', 'Mensaje');
	...
	{
		jAlert('<b>Por favor seleccione el campo</b>', 'Mensaje');
	}
}
//..........................................................................
function main_table(table)
{
    ...
    $('.cells').click(function()
    {
        ...
        {
            $(this).removeClass('table_sel');
            jAlert('La selección máxima de columnas no debe ser más de cinco','Mensaje');
...
function sel_class1(table)
{
    ...
    { 
        ...
        {
            jAlert('Al menos'+hleng+' las columnas necesitan ser seleccionadas','Message')
...
function sel_class(table)
{
    ...
    if($('body').width()<1030&&$('body').width()>600)
    {
        if($('.table_sel').length<3)
        {
            jAlert('Al menos 5 columnas deben ser seleccionadas','Mensaje');
            return false;
        } 
    }
    if($('body').width()>1030)
    {
        if($('.table_sel').length<5)
        {
            jAlert('Al menos 5 columnas deben ser seleccionadas','Mensaje',function(r)
...
function show_dilv(id,jonb,jum,back_to,table_div)
{
    ...
    doc_s +="<div class='blue_pop'>";
    doc_s +="<div class='red_pop'>";
 doc_s += "<div   class='exp_menu' name='"+back_to+"' id='display1' onclick='side_links(\"display1\",\""+table_div+"\")' alt='picking_and_post_goods/picking_and_post_goods?page=forms&url=picking_and_post_goods&values="+jonb+"&jum="+jum+"&titl=Pick And Post Goods&tabs=picking_and_post_goods'>Recoger y publicar bienes</div>";
doc_s += "<div   class='exp_menu' name='"+back_to+"' id='display1' onclick='side_links(\"display1\",\""+table_div+"\")' alt='picking_and_post_esign/picking_and_post_esign?page=forms&url=picking_and_post_goods&values="+jonb+"&jum="+jum+"&titl=Pick And Post eSign&tabs=picking_and_post_esign'>Elegir y publicar eSign</div>";
 doc_s += "<div class='exp_menu' id='Create_Billing1' onclick='pdfstrg(\""+ids[0]+"\")' >Salida de Nota de Entrega</div>";
 doc_s += "<div class='exp_menu' id='Create_Billing2' onclick='pdfstrgland(\""+ids[0]+"\")' >Salida de Conocimiento de Embarque</div>";
    doc_s += "</div>";
    doc_s +="</div>";
...
function show_purch(id,jonb,jum,back_to,table_div)
{
	...
	doc_s += "<div class='exp_menu' name='"+back_to+"' id='display1' onclick='side_links(\"display1\",\""+table_div+"\")' alt='edit_purchase_order/edit_purchase_order/?PURCHASE_ORDER="+ids[0]+"&titl=Display Purchase Order&key=edit_purchase_order'>Mostrar Orden de Compra</div>";
	doc_s += "<div class='exp_menu' id='Create_Billing1' onclick='pdfstrgpur(\""+ids[0]+"\")' >Salida de Orden de Compra</div>";
    doc_s += "</div>";
    doc_s +="</div>";
...
function show_prod(id, pnt, back_to,quotation)
{
	...
	doc_s +="<div class='red_pop'>";
    if(quotation == 0){
	   doc_s += "<div class='exp_menu' name='"+back_to+"' id='display1' onclick=\"show_prod_avail('"+id+"','"+pnt+"','"+back_to+"')\" >Disponibilidad de Producto</div>";
	}
    else{
     doc_s += "<div class='exp_menu' name='"+back_to+"' id='display1' onclick=\"show_prod_avail_quotation('"+id+"','"+pnt+"','"+back_to+"')\" >Disponibilidad de Producto</div>";   
    }
    doc_s += "</div>";
	doc_s +="</div>";
	var doc_h=$('#'+id).append(doc_s);
...
function show_prod_avail_quotation(id, pnt, back_to,creationPage=false)
{
    if(!creationPage){
        id = encodeURIComponent($("#"+id+" div").first().text());
    }
    ...
    $.ajax({
        type:'POST', 
        url: 'product_availability_quotation/product_avl?page=sublink&bapiName=ZEMG_BAPI_PRD_AVL&MATERIAL='+id+'&PLANT='+pnt, 
        success: function(response) {
            $('#loading').hide();
            $("body").css("opacity","1"); 
            $('.material_pop').html(response);
...
function show_rels(id,jonb,jum,back_to)
{
    ...
    */
    ...
    doc_s +="<div class='blue_pop'>";
    doc_s +="<div class='red_pop'>";
    doc_s += "<div class='exp_menu' id='display1' name='"+back_to+"' onclick='cert_deliv(\"display1\")' alt='release_prod_order/release_pro?bapiName=BAPI_PRODORD_RELEASE&page=sublink&url=releasepro&ORDER_NUMBER="+jonb+"&jum="+jum+"&titl=Release Prod Order&tabs=release_prod_order'>Lanzamiento</div>";
    doc_s += "</div>";
    doc_s += "</div>";
...
function approve_r(id,jonb)
{
    ...
    doc_s +="<div class='blue_pop'>";
    doc_s +="<div class='red_pop'>";
    doc_s += "<div class='exp_menu' id='approve1' name='"+back_to+"' onclick='cert_deliv(\"approve1\")' alt='approve_purchase_requisition/approve_request?bapiName=BAPI_REQUISITION_RELEASE_GEN&NUMBER="+jonb+"'>Aprobar Solicitud de Compra</div>";
    doc_s += "</div>";
    doc_s += "</div>";
    var doc_h=$('#'+id).append(doc_s);
    $('.red_pop').css({
...
{	
    ...
    doc_s +="<div class='blue_pop'>";
    doc_s +="<div class='red_pop'>";
    doc_s += "<div class='exp_menu' id='display1' name='"+back_to+"' onclick='side_links(\"display1\")' alt='confirm_prod_order/confirm_prod_order?page=forms&url=confirm_prod_order&json="+jon+"&value="+jonb+"&jum="+jum+"&titl=Confirm Prod Order&tabs=confirm_prod_order'>Confirmar Orden del Producto</div>";
    doc_s += "</div>";
    doc_s += "</div>";
    var doc_h=$('#'+tableID).find('#'+id).append(doc_s);
    $('.red_pop').css({
...
if(jum != 'DOCUMENT_FLOW_ALV_STRUC'){
    doc_s += "<div class='exp_menu' id='display2' name='"+back_to+"' onclick='side_links(\"display2\",\""+table_div+"\")' alt='document_flow/document_flow/?DOC_NUM="+ids[0]+"&titl=Document Flow&tabs=document_flow'>Flujo de Documentos</div>";
}
    doc_s += "<div class='exp_menu' id='Set_Delivery' onclick='cert_deliv(\"Set_Delivery\")' alt='common/setdeliveryblock/?REF_DOC="+ids[0]+"&bapiName=BAPI_SALESORDER_CHANGE&type=set'>Establecer Bloque de Entrega</div>";
    doc_s += "<div class='exp_menu' id='Remove_Delivery' onclick='cert_deliv(\"Remove_Delivery\")' alt='common/setdeliveryblock/?REF_DOC="+ids[0]+"&bapiName=BAPI_SALESORDER_CHANGE&type=remove'>Eliminar Bloque de Entrega</div>";
    doc_s += "<div class='exp_menu' id='Create_Delivery1' onclick='cert_deliv(\"Create_Delivery1\")' alt='common/createdelivery/?REF_DOC="+ids[0]+"&bapiName=BAPI_OUTB_DELIVERY_CREATE_SLS'>Crear Entrega</div>";
    doc_s += "<div class='exp_menu' onclick='cert_r(\""+ids[0]+"\",\""+id+"\")'>Liberación de Crédito</div>";
	doc_s += "<div class='exp_menu' onclick='cert_rr(\""+ids[0]+"\",\""+id+"\")'>Aprobar Orden de Venta</div>";
    if(jum=='/KYK/S_POWL_BILLDUE')
    {
        doc_s += "<div class='exp_menu' id='Create_Billing1' onclick='cert_deliv(\"Create_Billing1\")' alt='common/createbilling/?REF_DOC="+ids[0]+"&josn="+jonb+"&page=sublink&url=createbill&bapiName=ZBAPI_POWL_CREATE_BILLING'>Crear Facturación</div>";
    }
	}
    doc_s += "</div>";
    doc_s += "</div>";
...
function show_cus(id,back_to,table_div)
{
    ...
    doc_s += "<div class='exp_menu' id='Sales_Details1' name='"+back_to+"' onclick='side_links(\"Sales_Details1\",\""+table_div+"\")' alt='search_sales_orders/search_sales_orders?scr="+widt+"&bapiName=BAPI_SALESORDER_GETLIST&type=off&CUSTOMER_NUMBER="+ids[0]+"&SALES_ORGANIZATION=1000&titl=Search Sales Orders&tabs=search_sales_orders'>Detalles de Ventas</div>";
    doc_s += "<div class='exp_menu' id='Post_Incoming1' name='"+back_to+"' onclick='side_links(\"Post_Incoming1\",\""+table_div+"\")' alt='post_incoming_payment/post_incoming_payment?&bapiName=ZBAPI_ACC_DOCUMENT_POST&type=off&CUSTOMER="+ids[0]+"&titl=Post Incoming Payment&tabs=post_incoming_payment'>Pago de Entrada Entrante</div>";
    doc_s += "<div class='exp_menu' id='Create_Sales1' name='"+back_to+"' onclick='side_links(\"Create_Sales1\",\""+table_div+"\")' alt='create_sales_order/create_sales_order?&bapiName=BAPI_SALESORDER_CREATEFROMDAT2&type=off&CUSTOMER="+ids[0]+"&titl=Create Sales Order&tabs=create_sales_order'>Crear Orden de Venta</div>";
    doc_s += "</div>";
    doc_s += "</div>";
    var doc_h = $('#'+id).append(doc_s);
    var posiElement = parseInt($('#'+id).offset().top) - parseInt($(window).scrollTop());
...
function show_vendor_menu(id,back_to,table_div)
{
    ...
    doc_s += "<div class='exp_menu' id='Sales_Details1' name='"+back_to+"' onclick='side_links(\"Sales_Details1\",\""+table_div+"\")' alt='create_purchase_order/create_purchase_order?scr="+widt+"&type=off&vendor="+ids[0]+"&titl=Create Purchase Order&tabs=create_purchase_order'>Crear PO</div>";
...
function cert(id)
{
    ...
    doc_s +="<div class='blue_pop'>";
    doc_s +="<div class='red_pop'>";
    doc_s += "<div class='exp_menu' onclick='cert_r(\""+ids[0]+"\")'>Liberación de Crédito</div>";
...
function show_cus_map(id,back_to)
{
    ...
    doc_s += "<div class='exp_menu' id='Post_Incoming1' onclick='parent.side_links_map(\"post_incoming_payment/post_incoming_payment?page=forms&url=post_incoming_payment&bapiName=ZBAPI_ACC_DOCUMENT_POST&type=off&CUSTOMER="+ids[0]+"&titl=Post Incoming Payment&tabs=post_incoming_payment\")' alt=''>Pago de Entrada Entrante</div>";
    doc_s += "</div>";
    doc_s += "</div>";
    var doc_h=$('#'+id).append(doc_s);
    if(ure<=4)
...
function cert_r(ids,id)
{
    ...
    $.ajax({
        ...
        success: function(html) {
            ...
            $('#loading').hide();
            $("body").css("opacity","1"); 
            jAlert('<b>Mensaje del sistema SAP:</b><br>'+html, "Mensaje", function(r)
            {
                if(r)
                {
			...
			if(r)
			{
				...
				$.ajax({
					...
					success: function(html) 
					{
						...
						$('#loading').hide();
						$("body").css("opacity","1"); 
						jAlert('<b>Mensaje del Sistema SAP:</b><br>'+spt[0], "Mensaje", function() {
...
$('#loading').show();
		...
		$.ajax({
			...
			success: function(html) 
			{
				...
				$('#loading').hide();
				$("body").css("opacity","1"); 
				jAlert('<b>Mensaje del sistema SAP:</b><br>'+spt[0], "Menaje", function() {
					// this is the callback function of the jAlert box
					// we arrive here when the user has clicked "ok"
					// send him to google
					url_reload(resp_type);
...
function sear(id,val,ses)
{
	...
	$('#'+sess[1]+'_num').before( '<div id="srch_show_more" class="testr">Ver Más</div>' );
...
function serach_by(addr)
{
    ...
    $('.ser_by').remove();
    $('.appl').parent('div').parent('div').parent('div').parent('div').attr('class','rty');
    $('.gm-style-iw').after("<div style='border-left:1px solid #cecece;border-right:1px solid #cecece;background:#fff;width:292px;margin-top:139px;z-index:1000;position:absolute;' class='ser_by'><div style='margin-left:13px;padding-top:3px;'><input type='text' name='saer' class='sert' /><span class='go_sr'>Ir</span></div></div>");
...
function nearby_table() 
{
    ...
    $.ajax({
        ...
        success: function(html) 
        {
			...
			{
				$('#maps').html("<h4 style='color:#828282;'>Dirección no Encontrada</h4>");
				$('#maps').show();
				$('#tables').hide();
			}
        }
    ...
    $('.customize_input').each(function(index, element) {
        ...
        {      
            ...
            {    
                ...
                jAlert("Su cadena tiene caracteres especiales. \nEstos no están permitidos.","Mensaje");
    ...
    $('#tiwt_wel').keydown(function(e) {
        ...
        if(e.keyCode=='13')
        {
            ...
            $('#tweets').tweetable({
                ...
                failed: "Lo sentimos, Twitter no está disponible para este usuario.",
                html5: true,
        ...
        if(e.keyCode=='13')
        {
            ...
            if(url)
            {
                ...
                $.ajax({
					...
					if(html=='E')
					{
					 jAlert("Por favor, introduzca una URL válida","Mensaje",function(r){
                    if(r)
                    {
                        edit_url()
                    }
            ...
            {
                jAlert("Por favor, introduzca una URL válida","Mensage",function(r){
                    if(r)
                    {
                        edit_url()
                    }
...
function feed_url()
{
    ...
    $('.feed_in').keydown(function(e) {
        if(e.keyCode=='13')
        {
            ...
            {
                jAlert("Por favor, introduzca una URL válida","Mensaje");
                return false;
            }
