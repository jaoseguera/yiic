...
{   
    Createpdfview(table);
    jPrompt1('Email Id:', '', 'Send Email', function(r) {
        if( r ) 
        {
            CreateTable(table);            
            $('#loading').show();
...
function show_prod(id, pnt, back_to,quotation)
{
    ...
    if(quotation == 0){
     ...
     doc_s += "<div class='exp_menu' name='"+back_to+"' id='display1' onclick=\"show_prod_avail_quotation('"+id+"','"+pnt+"','"+back_to+"')\" >Product Availability</div>";   
    }	
	doc_s += "</div>";
	doc_s +="</div>";
	var doc_h=$('#'+id).append(doc_s);
