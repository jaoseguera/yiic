	 ...
	 var i=0;
	  $('.look_header').each(function(index, element) {
		...
		if($(this).hasClass('show_header'))
		{
			 ...
			 header_title +="<div alt='"+alts+"' class='sub_look active_look'>"+$(this).html()+"</div>";
		}
		else
		{
			header_title +="<div alt='"+alts+"' class='sub_look deactive_look'>"+$(this).html()+"</div>";
		}
		i++;
    });
	
	 $(this).after("<div class='div_look lookup_hed'>"+header_title+"</div>");
	 $('#lookup_head').mouseleave(function() {
        $('.div_look').hide();
    });
	$('.lup').click(function()
	{
	 ...
	 $('.sub_look').click(function() {
		var ids=$(this).attr('alt');
		var count=0;
		if($('.'+ids).hasClass('show_header'))
		{
			$(this).addClass('deactive_look').removeClass('active_look');
			$('.'+ids).addClass('hide_header').removeClass('show_header').hide();
	 ...
	 {
		 $(this).addClass('active_look').removeClass('deactive_look');
		 $('.'+ids).addClass('show_header').removeClass('hide_header').show();
	 }
		}
...
function save_db(order,type)
{

$.ajax({
		type:'POST', 
		url: "../lib/save_pre.php?order="+order+"&type="+type, 
...
function save_col(sdr,type)
{
	$.ajax({
		type:'POST', 
		url: "../lib/save_pre.php?display="+sdr+"&type="+type, 
		success: function(response) {
